...
> ParseInline<I> for InlineValue
{
    fn parse_inline(input: &mut I) -> object_rainbow::Result<Self> {
        ...
        Ok(match &*schema {
            ...
            InlineSchema::Map => Self::InlineSchema(input.parse_inline()?),
        })
    }
}
...
impl IsUnit for InlineSchema {
    fn is_unit(&self) -> bool {
        match self {
            Self::Never => false,
            Self::Unit => true,
            Self::Option(schema) if **schema == Self::Never => true,
            Self::Option(_) => false,
            Self::Point(_) => false,
            Self::Nt(_) => false,
            Self::Concat(a, b) => a.is_unit() && b.is_unit(),
            Self::Array((n, schema)) => *n == 0 || schema.is_unit(),
