...
#[proc_macro_derive(CanonicalExtra)]
pub fn derive_canonical_extra(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let name = input.ident;
    let generics = input.generics.clone();
    let (_, ty_generics, _) = generics.split_for_impl();
    let generics = match bounds_canonical_extra(input.generics, &input.data) {
        Ok(g) => g,
        Err(e) => return e.into_compile_error().into(),
    };
...
fn bounds_canonical_extra(mut generics: Generics, data: &Data) -> syn::Result<Generics> {
    match data {
        Data::Struct(data) => {
            if let Some(f) = data.fields.iter().next() {
                ...
                generics
                    ...
                    .predicates
                    .push(parse_quote_spanned! { ty.span() =>
                        #ty: ::object_rainbow::CanonicalExtra<Extra = __Extra>
                    });
            }
        }
        Data::Enum(data) => {
...
fn fields_canonicai_extra(
    ...
    match fields {
        syn::Fields::Named(fields) => {
            if let Some(f) = fields.named.iter().next() {
                let ident = f.ident.as_ref().unwrap();
                quote! {
                    { #ident, .. }
                ...
                syn::Error::new(span, "no fields").into_compile_error()
            }
        }
        syn::Fields::Unnamed(fields) => {
        ...
        syn::Fields::Unit => syn::Error::new(unit_span.unwrap(), "no fields").into_compile_error(),
    }
}
...
pub fn derive_for_wrapped(args: TokenStream, input: TokenStream) -> TokenStream {
...
pub fn pod(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let yes_clone = !no_clone.into_inner();
    let yes_copy = !no_copy.into_inner();
    let derive_clone = if yes_clone {
        quote! {
            ::core::clone::Clone,
        }
    } else {
    ...
    let derive_copy = if yes_clone && yes_copy {
        quote! {
            ::core::marker::Copy,
        }
    } else {
    ...
    let derive_tagged = if no_tagged.into_inner() {
        quote! {}
    } else {
        quote! {
