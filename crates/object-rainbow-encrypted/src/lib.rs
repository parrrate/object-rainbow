...
//! point encryption was added in as a proof-of-concept mechanism. The goal since the beginning was
//! to make encryption an "outside" capability: have core tools powerful enough to allow for such.
//! In process of making the original Python implementation, I effectively had to invent quite a lot
//! of things including custom dynamic tracing, since the trickiness of the whole resolution
...
//! have resolution translation be less messy (before that, it used to be just [`Hash`]). In this
//! actual project, main changes were in ways to forward keys. The now-removed first solution was
//! with dynamically typed extensions, which got replaced with so called "`Extra`", the system now
//! responsible for a lot of advanced functionality we have. Probably the only encryption-specific
//! concession which we just had to make (I did try doing it differently, didn't work) was mangle
//! hash. It is a bit of a tax on the project as a whole, so we are looking for some potentially
...
impl<K: Key, T: ToOutput> ToOutput for Encrypted<K, T> {
    fn to_output(&self, output: &mut (impl ?Sized + object_rainbow::Output)) {
