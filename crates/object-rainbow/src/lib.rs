...
pub trait InlineOutput: ToOutput {
    /// Specialisation point to allow types like [`u8`] to provide better serialisation
    /// implementation for containers like [`Vec<u8>`].
...
pub trait OptionOutput {
    /// Provide [`ToOutput::to_output`] for [`Option<Self>`].
...
pub trait Traversible: 'static + Sized + Send + Sync + FullHash + Topological {
    ...
    where
        Self: Clone,
    {
        Arc::new(crate::local_fetch::Local(self))
    }
}
...
pub trait SingularFetch: Singular + Fetch {
    ...
    where
        Self: Sized,
        Self::T: 'a + FullHash,
        Self: AsExtra<Extra: 'a + Send + Sync + ExtraFor<Self::T>>,
    {
        Box::pin(async move {
...
pub trait FromOutput: From<Self::Output> {
    type Output: Output;
}
